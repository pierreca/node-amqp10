...
/**
 * Determines whether the passed in object is a "plain" object
 */
u.isPlainObject = function(object) {
  return !!object && typeof object === 'object' && object.toString() === '[object Object]';
};

/**
 * Iterate over an objects values, not exactly a mapping but very fast
 */
u.objectMap = function(object, iterator) {
  var keys = Object.keys(object);
  var length = keys.length;
  for (var i = 0; i < length; ++i) {
    iterator(object[keys[i]], i);
  }
};
...
u.orNull = function(arg1) { return onUndef(arg1, null); };

u.orFalse = function(arg1) { return onUndef(arg1, false); };
...
// @todo: this could be dropped
u.contains = function(arr, elt) {
...
u.bufferEquals = bufferEquals;

// @todo: this "parsing" should be far more rigorous
u.parseLinkAddress = function(address, policy) {
...
u.deepMerge = deepMerge;
u.deepCopy = deepMerge;
...
u.generateTimeouts = function(options) {
 ...
 * @return {Object}
 */
u.dispositionRange = function(message) {
  var first, last;
 ...
 * @return {String}
 */
u.linkName = function(address, policyOverrides) {
 ...
 */
u.assignDefined = function(dest, source, properties) {
  var propertiesToAssign = u.omit(properties, function(value, key) {
    return source[key] === null || source[key] === undefined;
  });

  u.assign(dest, propertiesToAssign);
};
 ...
 */
u.assignFromDescribedType = function(Type, describedType, dest, defaults) {
  ...
  if (defaults !== undefined)
    u.defaults(dest, defaults);     // NOTE: this mutates the dest object
};
